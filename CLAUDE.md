...
- **Session registries are keyed maps whose entry is the ownership token.** Five of them exist (`termSessions`, `execSessions`, `logSessions`, `aiSessions`, plus the `cancelRegistry` instances in `controller/sessions.go`), and they share two rules:
  - **A stream's goroutine must delete its own entry, not just the Close function.** A shell that exits on its own, a pod that dies, a broken stream — none of them call `Close*Session`, so cleanup that lives only there leaks the entry, leaves the pty child unreaped (`cmd.Wait` in `terminalServices.go`'s reader goroutine is the only place a terminal is reaped) and leaves the panel silently unresponsive. Each such path also emits a terminal event — `terminal:exit:{id}`, `exec:closed:{id}`, `climode:exit:{id}` — so the frontend can distinguish a dead session from a quiet one.
